...
final class Config_Controller {
	...
	public static function get_config( WP_REST_Request $request ): WP_REST_Response {
		...
		$payload = array(
			...
			'nonces'          => Security::admin_nonces( $can_manage, true ),
		);

		if ( $can_manage ) {
			$payload['slackConnected']  = Settings::is_slack_connected();
			$payload['slackUserName']   = (string) ( $settings['slack_user_name'] ?? '' );
			$payload['slackAuthorize']  = OAuth_Client::get_authorize_url();
			$payload['slackRedirect']   = OAuth_Client::get_redirect_uri();
			$payload['hasSecret']       = '' !== $secret_stored;
			$payload['githubConnected'] = GitHub_OAuth_Client::is_connected();
			$payload['githubUserLogin'] = (string) ( $settings['github_user_login'] ?? '' );
			$payload['githubAuthorize'] = GitHub_OAuth_Client::get_authorize_url();
			$payload['githubRedirect']  = GitHub_OAuth_Client::get_redirect_uri();
			$payload['hasGithubSecret'] = '' !== $github_secret_stored;
			$payload['slackChannels']   = $slack_channels;
			$payload['users']           = array_map(
			...
			$payload['runLog']          = array_reverse( (array) ( $settings['run_log'] ?? array() ) );
			$payload['urls']['aiSettings'] = admin_url( 'options-general.php?page=ai-settings' );
